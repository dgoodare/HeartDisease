...
class Problem:
    ...
    def actions(self, node, action):
        ...
        #Checks each variable and determines the patient's overall risk level
        #from which it will return a list of appropriate actions for the patient

        #give an overall risk level
        riskLevel = node.state.getRiskLevel()
        #the list of actions
        actionList = []
...
def h(node):
    ...
    if (riskLevel <= 2):
        #patient is at low risk, on average will recover after 1 action (3 months)
        #for admissability this is the step cost for 1 medium intensity action
        return 3
    elif ((riskLevel >= 3) and (riskLevel <= 5)):
        #patient is at medium risk, on average will recover after 3 actions (9 months)
        #for admissability this is the step cost for 3 low intensity action
        return 6
    elif ((riskLevel >= 6)):
        #patient is at high risk, on average will recover after 8 actions (24 months)
        #this is the step cost for 8 low intensity actions
        return 16
